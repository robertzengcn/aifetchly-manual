...
description: aiFetchlyのブラウザパス、CAPTCHAソルバー、システム設定を構成します。
---
...
**フック**は、AI アシスタントのツール呼び出しの前後でカスタムロジックを実行するライフサイクルトリガーです。各呼び出しを監視、ブロック、書き換え、注釈できます。aiFetchly は組み込みの安全フック（例：危険なシェルの削除をブロック）を同梱しており、さらにポリシーを強制し、コンテキストを追加し、監査ログを保持する独自のコマンドフックを追加できます。サブシステム全体はこのページからグローバルに切り替えできます。

イベント、マッチャー、コマンドフック、監査ログに関する完全なドキュメントについては、[フック](./hooks) を参照してください。

## AI プロバイダー

**AI プロバイダー**ページでは、AI チャットがモデルを取得する場所を選択できます。aiFetchly の **ホステッド** AI（サブスクリプションに含まれる）または独自の **カスタム / ローカルプロバイダー** です。Ollama、LM Studio、OpenAI、OpenRouter、vLLM、LocalAI、カスタムサーバーなど、任意の OpenAI 互換エンドポイントを aiFetchly に指定することで、aiFetchly AI サブスクリプションがなくても独自のモデルで AI チャットを利用できます。プロバイダープリセット、ベース URL、オプションの API キー、デフォルトモデルを構成し、モデルを更新して接続テストを実行すると、検出された機能（models、chat、streaming、tools、vision）が確認できます。API キーは保存時に暗号化され、保存後は再表示されません。ホステッド限定の AI 機能（キーワード生成、メールテンプレート、リランク、埋め込み）は、この設定に関係なく引き続きサブスクリプションが必要です。

プロバイダーモード、プリセット、接続テスト、セキュリティに関する完全なドキュメントについては、[AI プロバイダー](./ai-provider) を参照してください。
