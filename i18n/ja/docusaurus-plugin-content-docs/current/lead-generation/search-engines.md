...
この機能は公開Web情報を自動的に整理する目的でのみ使用してください。Market Insight の利用が対象サイトの Robots プロトコルと利用規約に従っていることを確認してください。
...
| **Bing** | 一般検索、Microsoft Search のカバレッジ | 推奨 | 任意 |
...
2. **Market Insight** カードを選択
3. Market Insight のタスクリストが表示されます
4. **Create Search Task** をクリックして検索フォームを開きます
...
3. **必須**: Yandex スクレイピング
4. **推奨**: 大規模な Google および Bing スクレイピング
...
強力なアンチボット保護を持つエンジン（Google や Bing など）のスクレイピング時や、大規模タスクで時折エラーが予想される場合に AI リカバリーを有効にしてください。
...
左側のナビゲーションメニューで **Insights** を開き、**Market Insight** を選択してすべての検索タスクを表示します。
...
| **Search Engine** | 使用エンジン（Google、Bing、Yandex） |
...
1. 左側のナビゲーションメニューで **Insights** を開き、**Market Insight** を選択
2. タスクを見つける
...
| **Contact Profile Insights** | 連絡先情報の抽出ステータス |
...
- **Open in Contact Profile Insights**: 選択した URL をメール抽出機能に渡す
...
2. **Open in Contact Profile Insights** ボタンをクリック
...
左側のナビゲーションメニューで **Insights** を開き、**Contact Profile** を選択して収集されたメールを表示します。
...
**Bing:**
- 大規模タスクでは認証済みアカウントを使用
- スケールアップ時はローカルブラウザを有効化
- 堅牢なエラー処理のために AI リカバリーを有効化
- レート制限を尊重（1 同時実行から開始）
